...
MFCCFunction <-
  function(input.dir, min.freq = 200, max.freq = 10000,
    ...
    call.timing.list <- list.files(input.dir,full.names = T,pattern='.wav',recursive = T)
    
    call.timing.list.short <- list.files(input.dir,full.names = F,pattern='.wav',recursive = T)
